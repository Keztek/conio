...
<body>
    ...
    <div class="nonfooter">
        ...
        <div class="focuscontainer">
        	<div class="focus">
        		<h1>ConIO SDK</h1>
        		<p>A single download contains the entire Software Development Kit that you need to get started.</p>
        		<ul>
        			<li>Javascript Client Library</li>
        			<li>.Net 8 Client Library<!--<span style="font-size:smaller;color:red;font-weight:bold;margin-left:5px;vertical-align:top">New!</span>--></li>
        			<li>Multiplayer Development Server</li>
        		</ul>
        		<p><a href="/termsofservice">Terms</a> Apply.</p>
                <?php
                
                $sql = "SELECT * FROM sdk ORDER BY id DESC LIMIT 1";
                $result = $con->query($sql);

                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $phpdate = strtotime($row['released']);
                        $mysqldate = date('M d Y', $phpdate);
                        echo '<a target="_blank" href="'.$row['file'].'" class="sdkbutton">Download SDK<span>v'.$row['version'].', released '.$mysqldate.'</span></a>';
                    }
                } else {
                    echo '<a href="#" class="sdkbutton">Download SDK<span>Not available yet</span></a>';
                }
